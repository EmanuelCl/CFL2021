...
let opcion: string = RLS.question('Ingrese una opcion,(C=CREAR),(R=BUSCAR),(U=MODIFICAR),(D=ELIMINAR),(S=CALCULAR SEGURO) X para finalizar: ').toUpperCase();
let patente: string;
while (opcion != 'X') {
    switch (opcion) {
        case 'C': {
        ...
        case "S": {
            registro.cotizarSeguro();
            break;
        }   
    }
    registro.mostrarVehiculo();
    opcion = RLS.question('Ingrese una opcion,(C=CREAR),(R=BUSCAR),(U=MODIFICAR),(D=ELIMINAR),(S=CALCULAR SEGURO) X para finalizar: ').toUpperCase();
}
